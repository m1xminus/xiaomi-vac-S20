...
class IjaiVacuumDevice:
    ...
    def clean_segments(self, room_ids: list[int | str]) -> None:
        if not room_ids:
            # See vacuum.async_clean_segment's guard for why — an empty list
            # silently becomes a full-home clean on this hardware, not a no-op.
            raise ValueError("clean_segments called with no room ids")
    ...
    def room_clean_pause_params(self) -> tuple[object, list] | None:
        """Same action as room_clean_set_params, but oper=2 (Pause) with an
        EMPTY room-ids string — captured directly from the real Mi Home
        app's own network traffic (not inferred, not guessed): pausing an
        ALREADY-RUNNING room job sends `in=["", 0, 2]`, not the room ids
        that were used to start it. Confirmed this genuinely matters: with
        an empty room-ids string, the device's status property holds at
        raw value 2 (Paused) across multiple subsequent polls, matching
        the app's own "a interromper a limpeza" behavior exactly. Every
        earlier attempt at this re-sent the actual room ids (e.g.
        `in=["16", 0, 2]`) — that version never got the device to report
        Paused at all; it went straight to Idle instead, no matter which
        oper value was tried. Room ids are apparently only meaningful when
        STARTING a job fresh (room_clean_set_params) — addressing an
        already-active job for pause/resume uses an empty string, letting
        the device operate on whatever job it already has in its own
        internal state.
        """
        cap = self.profile.room_clean
        if not (
            ...
            return None
        values = {
            cap.clean_room_mode.piid: 0,
            cap.clean_room_oper.piid: 2,  # pause
            cap.clean_room_ids.piid: "",  # empty — see docstring, confirmed from real Mi Home traffic
        }
        return cap.set_room_clean, [values[piid] for piid in cap.set_room_clean.in_piids]

    def room_clean_resume_params(self) -> tuple[object, list] | None:
        """Same action, oper=1 (Start) with an EMPTY room-ids string — the
        real Mi Home resume call, captured directly from its own traffic:
        `in=["", 0, 1]`. This is NOT the same as room_clean_set_params
        (which sends real room ids and is for starting a brand new job) —
        resuming an already-paused job addresses it with an empty string,
        same as pausing does. Confirmed on real hardware this is what
        makes the device's status flip back to Cleaning and continue from
        near its last position, rather than restarting the job.
        """
        cap = self.profile.room_clean
        if not (
            cap is not None
        ...
        values = {
            cap.clean_room_mode.piid: 0,
            cap.clean_room_oper.piid: 1,  # start/resume
            cap.clean_room_ids.piid: "",  # empty — see docstring, confirmed from real Mi Home traffic
        }
        return cap.set_room_clean, [values[piid] for piid in cap.set_room_clean.in_piids]
