...
const MODE_OPTIONS = [
  { value: 0, label: "Sweep" },
  { value: 1, label: "Sweep + Mop" },
  { value: 2, label: "Mop" },
  { value: 3, label: "Sweep then Mop" },
];
const POWER_OPTIONS = [
  { value: 0, label: "Silent" },
  { value: 1, label: "Basic" },
  { value: 2, label: "Strong" },
  { value: 3, label: "Full Speed" },
];
const WATER_OPTIONS = [
  { value: 0, label: "0" },
  { value: 1, label: "1" },
...
class XiaomiVacCard extends HTMLElement {
    ...
    this._root.innerHTML = `
      ...
      <div class="room-sheet">
        ...
        <div class="sheet-group" data-field="wind_power">
          <div class="sheet-label">Suction</div>
          <div class="sheet-opts"></div>
        </div>
        <div class="sheet-group" data-field="water_level">
          <div class="sheet-label">Water</div>
          <div class="sheet-opts"></div>
        </div>
        <div class="sheet-actions">
          <button class="sheet-remove">Remove</button>
          <button class="sheet-confirm">Add to clean</button>
        </div>
      </div>
      <div class="presets-sheet">
        <div class="sheet-title">Presets</div>
      ...
      <div class="tray">
        ...
        <button class="b open-presets" title="Presets" aria-label="Open presets"><ha-icon icon="${MDI.presets}"></ha-icon></button>
      </div>`;
    this.appendChild(this._root);
  ...
  _refreshFill() {
    ...
    this._root.querySelectorAll(`.pg-map[data-mi="${this._real}"] path.rm[data-haschain="0"]`)
      .forEach((p) => {
        ...
        p.setAttribute("fill", this._sel.has(id) ? p.dataset.tint1 : p.dataset.tint0);
      });
  }
  ...
  _mapSVG(m) {
    ...
    polys.forEach((p) => {
      ...
      // raster covers chain-precision rooms -> transparent hit target;
      // bbox-fallback rooms have no raster coverage -> paint the tint directly
      const fill = (raster && p.hasChain) ? "transparent" : t[0];
      s += `<path class="rm" data-id="${p.id}" data-haschain="${p.hasChain ? 1 : 0}" data-tint0="${t[0]}" data-tint1="${t[1]}" role="button" tabindex="0" aria-pressed="false" aria-label="Clean ${esc(nm)}" d="${dpath(p.rings)}" fill="${fill}" stroke="none"/>`;
    });
  ...
  _cleanSelected() {
    ...
    // apply_room_preferences service), THEN start the clean. Sequenced with
    // await so a slow preference write can't race the clean-start call.
    (async () => {
      try {
        ...
        console.error("[xiaomi-vac-card] apply_room_preferences failed:", e);
      }
  ...
  _openRoomSheet(id) {
    ...
    const current = this._roomSettings[id] || { ...ROOM_SETTING_DEFAULTS };
    this._sheetPending = { ...current };
    const q = (s) => this._root.querySelector(s);
    q(".sheet-title").textContent = this._roomName(id);
    q(".sheet-remove").classList.toggle("show", this._sel.has(id));
    const groups = [
    ...
    groups.forEach(([field, opts]) => {
      ...
      wrap.innerHTML = opts.map((o) =>
        `<button type="button" class="sheet-opt${this._sheetPending[field] === o.value ? " on" : ""}" data-value="${o.value}">${esc(o.label)}</button>`
      ).join("");
  ...
  _removeRoomSheet() {
    if (this._roomPopupId == null) return;
    this._sel.delete(this._roomPopupId);
    delete this._roomSettings[this._roomPopupId];
    this._closeRoomSheet();
    this._syncRooms();
  }
  ...
  _openPresets() {
    ...
    list.innerHTML = presets.length
      ...
      : `<div class="presets-empty">No presets configured yet. Add them in the card's visual editor.</div>`;
    ...
    if (!this._form) {
      ...
      this._form.computeLabel = (s) =>
        ({
          ...
          mode: "Cleaning mode select entity",
          ...
          presets: "Presets (list of {name, script})",
        }[s.name] || s.name);
